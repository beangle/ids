...
/** OAuth2 授权码。
 *
 *  记录授权码流程中的授权信息，支持 PKCE (RFC 7636)。
 *  不持久化到数据库，设计用于 Redis 存储，通过 JSON 序列化。
 *
 *  @param code 授权码，用户授权后生成，用于换取 access token
 *  @param clientId 客户端编码
 *  @param userId 用户编码
 *  @param scope 授权范围，多个用空格分隔
 *  @param codeChallenge PKCE 的 code_challenge，token 交换时用于验证 code_verifier
